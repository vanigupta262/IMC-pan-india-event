...
def run_bot_in_sandbox(bot_path, snapshot, timeout=2.0, memory="256m", cpus=0.5, image="igts-bot-sandbox"):
    ...
    # Docker run command
    # Note: --cpus removed for WSL compatibility (kernel doesn't support CPU CFS scheduler)
    docker_cmd = [
        "docker",
        "run",
        "--rm",
        ...
        "--memory", memory,
        # "--cpus", str(cpus),  # Disabled for WSL compatibility
        "--read-only",
        "--tmpfs", "/tmp:size=10m",
        "--network", "none",
    ...
    try:
        ...
        # Include raw output in error for debugging
        msg = f"Bot {bot_path.name} returned invalid JSON: {bot_output[:100]}..." 
        raise json.JSONDecodeError(msg, bot_output, 0)
