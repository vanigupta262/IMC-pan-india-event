...
Usage:
  python3 dev_tools/test_system.py [--verbose] [--skip-docker]
"""
import subprocess
import json
import sys
...
class SystemTest:
    def __init__(self, verbose=False, skip_docker=False):
        ...
        self.results = []
        # root should be repository root (one level up from this script)
    ...
    def test_engine_compile(self):
        ...
        assert Path(self.root / "game").exists(), "game binary not created"
    ...
    def test_match_runner_creates_log(self):
        ...
        # Check log exists
        log_path = Path(self.root / "logs" / "match_1.json")
    ...
    def test_action_matrix_flow(self):
        ...
        if Path(self.root / "actions.txt").exists():
            (Path(self.root / "actions.txt")).unlink()
        ...
        actions_file = Path(self.root / "actions.txt")
