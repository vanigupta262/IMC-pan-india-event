...
async def login(login_data: LoginRequest, db: Session = Depends(get_db)):
    ...
    # In a real app, verify hash. Here we compare plain text as per Register implementation
...
async def upload_bot(
    ...
    # Deactivate all previous bots for this user
    previous_bots = db.query(Bot).filter(Bot.user_id == user_id, Bot.is_active == True).all()
    for pb in previous_bots:
        pb.is_active = False
...
async def list_bots(user_id: int, db: Session = Depends(get_db)):
    ...
    bots = db.query(Bot).filter(Bot.user_id == user_id).order_by(Bot.id.desc()).all()
    return bots
...
async def activate_bot(bot_id: int, db: Session = Depends(get_db)):
    ...
    # Deactivate other bots for this user
    db.query(Bot).filter(Bot.user_id == bot.user_id).update({"is_active": False})
...
async def create_lobby(lobby_data: LobbyCreate, db: Session = Depends(get_db)):
    ...
    # Max players validation removed - now configurable per lobby
    if lobby_data.max_players > 10:  # Reasonable upper limit
        raise HTTPException(status_code=400, detail="Lobby max_players cannot exceed 10")
    ...
    if lobby:
        ...
        for member in members:
            ...
            if user:
                ...
                # SELECT LATEST ACTIVE BOT
                bot = db.query(Bot).filter(Bot.user_id == user.id, Bot.is_active == True).order_by(Bot.id.desc()).first()
                if bot:
        ...
        if game_log.get("final_state") and game_log["final_state"].get("rankings"):
            for result in game_log["final_state"]["rankings"]:
                ...
                if user_id:
                    # SELECT LATEST ACTIVE BOT
                    bot = db.query(Bot).filter(Bot.user_id == user_id, Bot.is_active == True).order_by(Bot.id.desc()).first()
                    if bot:
