...
class GameOrchestrator:
    ...
    def __init__(self, match_id: int, players: List[Dict[str, Any]], bots: Dict[int, str], 
                 enable_black_swan: bool = True, black_swan_seed: int = None):
        """
        ...
        self.black_swan = BlackSwanEvent(
            num_rounds=self.NUM_ROUNDS,
            enabled=enable_black_swan,
            seed=black_swan_seed
        )
        if self.black_swan.enabled:
            print(f"[Match {match_id}] Black Swan enabled - will trigger at round {self.black_swan.event_round}")
    ...
    def _write_actions_file(self, actions_matrix: list):
        ...
        with open("actions.txt", "w") as f:
            for player_id, actions in enumerate(player_actions):
                # Format: code:target code:target ...
                # actions for thisplayer is a list of (code, target) tuples now? 
                # Wait, the loop above needs adjustment first.
                pass
            
            # Re-implementing the loop cleanly
            for player_id, round_actions in enumerate(actions_matrix):
                # This loop logic in original file was transposed implicitly or explicitly?
                # Original: for player_id, enumerate(player_actions) ... where player_actions was transposed
                pass

        # Correct logic:
        # actions_matrix is [round][player] = action_dict
        
        # Transpose to [player][round] = "code:target" string
        player_lines = [[] for _ in range(self.num_players)]
        
        for round_actions in actions_matrix:
            for player_id, action in enumerate(round_actions):
                action_type = action.get("type", "NO_OP")
                action_code = action_type_map.get(action_type, 6) # 6 is NO_OP
                target = action.get("target", -1)
                
                token = f"{action_code}:{target}"
                player_lines[player_id].append(token)
