...
class GameOrchestrator:
    ...
    NUM_ROUNDS = 100  # Tournament standard (configurable per match)
    ...
    def __init__(
        ...
        Args:
            ...
            enable_black_swan: Whether to enable Black Swan event (default: True)
            black_swan_seed: Random seed for Black Swan (optional, for reproducibility)
        """
        ...
        self.game_log = {
            ...
            "rounds": [],
            "final_state": None,
            "black_swan": None,  # Will be populated if event triggers
        }
        ...
        if self.black_swan.enabled:
            print(
                f"[Match {match_id}] Black Swan enabled - will trigger at round {self.black_swan.event_round}"
            )
            ...
            if hasattr(bot_module, "get_action"):
                ...
                if target == -1 and self.num_players > 1:
                    ...
                    target = random.choice(
                        [p for p in range(self.num_players) if p != player_id]
                    )
                return {"type": action_type, "target": target}
    ...
    def run_round(self, round_num: int) -> Dict:
        ...
        """
        round_log = {"round": round_num + 1, "actions": [], "events": []}
        ...
        for player_id in self.player_economies:
            delta = sum(
                -50 if all_actions.get(player_id, {}).get("type") != "NO_OP" else 0
                for _ in range(1)
            )
            self.player_economies[player_id] = max(
                100, self.player_economies[player_id] + 50 + delta
            )
    ...
    def run(self) -> Dict:
        ...
        """
        print(
            f"[Match {self.match_id}] Starting game with {self.num_players} players for {self.NUM_ROUNDS} rounds"
        )
        ...
        try:
            ...
            result = subprocess.run(
                [self.ENGINE_PATH], capture_output=True, text=True, timeout=30
            )
            ...
            print(
                f"[Match {self.match_id}] Engine executable not found at {self.ENGINE_PATH}"
            )
    ...
    def _collect_bot_actions_for_all_rounds(self) -> list:
        ...
        print(f"[DEBUG] Simulated state keys: {list(simulated_state.keys())}")
        print(f"[DEBUG] Simulated state keys: {list(simulated_state.keys())}")
        ...
        for round_num in range(self.NUM_ROUNDS):
            ...
            for player_id in range(self.num_players):
                ...
                print(
                    f"[Match {self.match_id}] Round {round_num+1}, Player {player_id}: {action}"
                )
    ...
    def _write_actions_file(self, actions_matrix: list):
        ...
        # Prepare per-player lines. actions_matrix is [round][player]
        # Ensure we produce NUM_ROUNDS tokens per player; fill missing rounds with NO_OP
        default_token = f"{action_type_map['NO_OP']}:-1"
        player_lines = [
            [default_token for _ in range(self.NUM_ROUNDS)]
            for _ in range(self.num_players)
        ]

        for round_idx, round_actions in enumerate(actions_matrix):
            ...
            for player_id, action in enumerate(round_actions):
                if player_id >= self.num_players:
                    continue
                action_type = action.get("type", "NO_OP")
                action_code = action_type_map.get(action_type, action_type_map["NO_OP"])
                target = action.get("target", -1)
                token = f"{action_code}:{target}"
                player_lines[player_id][round_idx] = token
        ...
        print(
            f"[Match {self.match_id}] Wrote {len(player_lines)} player action sequences to actions.txt"
        )
    ...
    def _parse_engine_output(self, output: str) -> Dict:
        ...
        if player_states:
            rankings = sorted(
                [
                    {"player_id": p["id"], "economy": p["economy"], "rank": 0}
            ...
            game_log["final_state"] = {
                "player_economies": {
                    p["id"]: p["economy"] for p in player_states.values()
                },
    ...
    def _create_error_log(self, error_msg: str) -> Dict:
        ...
        return {
            ...
            "num_players": self.num_players,
            "num_rounds": self.NUM_ROUNDS,  # Fixed: was 0
            "start_time": datetime.utcnow().isoformat(),
