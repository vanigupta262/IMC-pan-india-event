...
// Bot Templates
const BOT_TEMPLATES = {
    basic: `# Basic Trading Bot Template
...
`,
    advanced: `# Advanced Trading Bot Template
# Implements multiple indicators and risk management
...
class AdvancedTradingBot:
    ...
    def trade(self, state):
        ...
        sma_short = self.calculate_sma(history, 5)
        sma_long = self.calculate_sma(history, 20)
        ...
        # Position sizing
        trade_amount = int(self.cash * self.risk_per_trade / price)
        trade_amount = max(1, min(trade_amount, 10))
        
        # Execute
        if buy_signal and self.position < self.max_position:
            return 'BUY', trade_amount
        elif sell_signal and self.position > 0:
            return 'SELL', min(trade_amount, self.position)
